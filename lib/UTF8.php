...
class UTF8 {
  ...
  static function smartUtf8_decode($inStr)
  {
    // Replace ? with a unique string
    $newStr = str_replace("?", "w0i0k0i0n0d0x", $inStr);
    // Try the utf8_decode
    $newStr = UTF8::decodeUtf8($newStr);
  ...
  static function decodeUtf8 ($utf8_string) {
  ...
  static function encodeUtf8($str) {
    preg_match_all("/&#([0-9]*?);/", $str, $unicode);
    foreach( $unicode[0] as $key => $value) {
      $str = preg_replace("/".$value."/", UTF8::code2utf8($unicode[1][$key]), $str);
    }
    return $str;		
  ...
  static function utf8_strtolower($string){
    ...
    $uni = UTF8::utf8_to_unicode($string); 
    $cnt = count($uni);
    for ($i=0; $i < $cnt; $i++){
      if(array_key_exists($uni[$i], UTF8::$UTF8_UPPER_TO_LOWER) && UTF8::$UTF8_UPPER_TO_LOWER[$uni[$i]]){
	$uni[$i] = UTF8::$UTF8_UPPER_TO_LOWER[$uni[$i]];
      }
    }
    return UTF8::unicode_to_utf8($uni);
  }
  ...
  static function utf8_strtoupper($string){
    ...
    for ($i=0; $i < count($uni); $i++){
      if(array_key_exists($uni[$i], UTF8::$UTF8_LOWER_TO_UPPER) && UTF8::$UTF8_LOWER_TO_UPPER[$uni[$i]]){
	$uni[$i] = UTF8::$UTF8_LOWER_TO_UPPER[$uni[$i]];
      }
    }
    return UTF8::unicode_to_utf8($uni);
  }
  ...
  static function utf8_htmlspecialchars ($str) {
    $str = str_replace("\"", "&quot;", $str);    
    $str = str_replace("<", "&lt;", $str);    
    $str = str_replace(">", "&gt;", $str);    
...
UTF8::$UTF8_UPPER_TO_LOWER = array_flip(UTF8::$UTF8_LOWER_TO_UPPER);
